...
begin
l_help := 'Procedura per l''invio di Notifiche push - APEX-net srl';
l_help := l_help ||'rel.1.0 - S. Teodorani';
l_help := l_help ||'-------------------------------------------------------------';
l_help := l_help ||'Esempio di utilizzo: ';
l_help := l_help ||'set serveroutput on';
l_help := l_help ||'beginv';
l_help := l_help ||'declare ';
l_help := l_help ||'   a varchar(2000);';
l_help := l_help ||' begin';
l_help := l_help ||'   a:= NOTIFICATORE.SENDNOTIFICATION(''Testo del messaggio'', ''NomeUtente'', 6);';
l_help := l_help ||'   dbms_output.put_line(a);';
l_help := l_help ||'end;';
l_help := l_help ||'/';
l_help := l_help ||'Per compilare la procedura eseguire:';
l_help := l_help ||'wrap iname=test.sql oname=test_wrap.sql';
 ...
 */
 ...
 is
  ...
  l_AuthKey           varchar2(36) :=  p_authkey;
  l_AppCode           varchar2(100) := p_authcode;
...
BEGIN
  ...
  if trim(p_MessageEng) is null then
     raise_application_error(-20002, 'Errore: Il Messaggio deve essere valorizzato');
  end if;
  ...
  l_payload          :=
    '<prog:SendNotificationWithLang soapenv:encodingStyle="http://schemas.xmlsoap.org/soap/encoding/">
         <pAuthKey xsi:type="xsd:string">'|| l_AuthKey || '</pAuthKey>
         <pApplicationKey xsi:type="xsd:string">'|| l_AppCode || '</pApplicationKey>
         <pMessage xsi:type="xsd:string"> {"messages":[ { "lang":"it", "message":"'||p_Message||'"},{"lang":"en", "message":"'||p_MessageEng||'"}  ]}</pMessage>
         <pUserName xsi:type="xsd:string">' || p_Username || '</pUserName>
         <pSound xsi:type="xsd:string">'|| l_SoundName || '</pSound>
         <pBadge xsi:type="xsd:int">'|| l_Badge || '</pBadge>
      </prog:SendNotificationWithLang>';
      
      dbms_output.put_line (l_payload);
      
  l_response_payload := soap_call(l_payload, l_target_url, 'http://www.progamma.com/SendNotificationWithLang');
