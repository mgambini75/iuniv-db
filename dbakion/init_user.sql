/* Formatted on 08/03/2016 17:18:55 (QP5 v5.287) */
CREATE VIEW V_K_PRODUCT
AS
   SELECT 'IUNIV' PRODUCT FROM DUAL;


CREATE TABLE K_MOD_VER
(
   ID               NUMBER (10) NOT NULL,
   MODULE           VARCHAR2 (10) NOT NULL,
   VERSION          VARCHAR2 (20) NOT NULL,
   DATE_RELEASE     DATE NOT NULL,
   DATE_APPL        DATE DEFAULT SYSDATE NOT NULL,
   ASSEMBLED        NUMBER (6) NULL,
   INTERNAL_BUILD   NUMBER (10) NULL,
   APPLICATION      NUMBER (1) DEFAULT 1 NOT NULL,
   VERSION_NUM      NUMBER (10) NOT NULL
);



CREATE UNIQUE INDEX PK_K_MOD_VER
   ON K_MOD_VER (ID)
   NOLOGGING;

ALTER TABLE K_MOD_VER
   ADD CONSTRAINT PK_K_MOD_VER PRIMARY KEY (ID) USING INDEX;


CREATE UNIQUE INDEX XAK_KMODVER_MOD_VERNUM
   ON K_MOD_VER (MODULE, VERSION_NUM)
   NOLOGGING;

ALTER TABLE K_MOD_VER
   ADD CONSTRAINT XAK_KMODVER_MOD_VERNUM UNIQUE (MODULE, VERSION_NUM)
       USING INDEX;



CREATE SEQUENCE S_K_MOD_VER START WITH 1
...
CREATE OR REPLACE TRIGGER K_MOD_VER_ROW_TBI
   BEFORE INSERT
   ON K_MOD_VER
   FOR EACH ROW
DECLARE
...
BEGIN
   -- popola il progressivo se non valorizzato


   IF (:NEW.ID IS NULL)
   THEN
      SELECT s_k_mod_ver.NEXTVAL INTO l_nextval FROM DUAL;

      :NEW.ID := l_nextval;
   END IF;

   IF (:NEW.INTERNAL_BUILD IS NULL AND INSTR (:NEW.VERSION, 'r') > 0)
   THEN
      :NEW.INTERNAL_BUILD :=
         SUBSTR (:NEW.VERSION,
                 INSTR (:NEW.VERSION, 'r') + 1,
                 LENGTH (:NEW.VERSION));
   END IF;
END K_MOD_VER_ROW_TBI/*******************************************************************************
                     $Id: KION_Oracle.py,v 1.13 2016/03/08 16:01:28 taguser Exp $
                     COPYRIGHT 2015 KION BY CINECA
                     All Rights Reserved.
                     valorizza il progressivo in caso di inserimento, aggiorna la data di applicazione in caso di modifica, assegna al modulo il valore del prodotto qualora non sia valorizzato gestisce la build interna
                     *******************************************************************************/
;
/


INSERT INTO K_MOD_VER (MODULE,
                       VERSION,
                       VERSION_NUM,
                       DATE_RELEASE)
...
CREATE VIEW V_K_SCHEMA_INFO
AS
   SELECT PROD.PRODUCT,
          VER_MAX.VERSION,
          'KION' CUSTOM_COD,
          0 CUSTOM_NUM
     FROM K_MOD_VER VER_MAX
          INNER JOIN V_K_PRODUCT PROD ON (VER_MAX.MODULE = PROD.PRODUCT)
    WHERE VER_MAX.VERSION_NUM = (SELECT MAX (VER.VERSION_NUM) VER_NUM
                                   FROM K_MOD_VER VER
                                  WHERE VER.MODULE = VER_MAX.MODULE);
...
(
   ID             NUMBER (10) NOT NULL,
   VER_ID         NUMBER (10) NOT NULL,
   FIX_TYPE       NUMBER (1) NOT NULL,
   FIX_DES        VARCHAR2 (255) NOT NULL,
   DATE_APPL      DATE NOT NULL,
   EXECUTED_FLG   NUMBER (1) DEFAULT 0 NOT NULL
);


COMMENT ON TABLE K_FIX IS 'It contains application type fix';

CREATE UNIQUE INDEX PK_K_FIX
   ON K_FIX (ID)
   NOLOGGING;

ALTER TABLE K_FIX
   ADD CONSTRAINT PK_K_FIX PRIMARY KEY (ID) USING INDEX;

CREATE UNIQUE INDEX XAK_KFIX_VER_TYPE_DES
   ON k_fix (ver_id, fix_type, fix_des)
   NOLOGGING;


ALTER TABLE K_FIX
   ADD CONSTRAINT XAK_KFIX_VER_TYPE_DES UNIQUE (ver_id, fix_type, fix_des)
       USING INDEX;


CREATE SEQUENCE S_K_FIX START WITH 1
                        MAXVALUE 999999999999
                        MINVALUE 0
                        NOCYCLE
                        NOCACHE
...
CREATE OR REPLACE TRIGGER K_FIX_ROW_TBI
   BEFORE INSERT
   ON K_FIX
   FOR EACH ROW
DECLARE
...
BEGIN
   IF (:NEW.ID IS NULL)
   THEN
      SELECT S_K_FIX.NEXTVAL INTO l_nextval FROM DUAL;
...
CREATE TABLE K_RESULT_FIX
(
   MODULE         VARCHAR2 (30),
   VERSION        VARCHAR2 (20),
   DATE_APPL      VARCHAR2 (50),
   FNAME          VARCHAR2 (255),
   RESULT         VARCHAR2 (4000),
   N_ERROR        NUMBER (3),
   VALIDATE_FLG   NUMBER (1) DEFAULT 1 NOT NULL,
   DURATION       NUMBER (10)
);

CREATE UNIQUE INDEX XPK_KRESULTFIX
   ON K_RESULT_FIX (MODULE,
                    VERSION,
                    DATE_APPL,
                    FNAME)
   NOLOGGING;

ALTER TABLE K_RESULT_FIX
   ADD CONSTRAINT XPK_KRESULTFIX PRIMARY KEY
          (MODULE,
           VERSION,
           DATE_APPL,
           FNAME)
...
CREATE TABLE k_job_custom_to_restart
(
   job         NUMBER NOT NULL,
   next_date   DATE NOT NULL
);

CREATE UNIQUE INDEX xpk_job_custom_to_restart
   ON k_job_custom_to_restart (job)
   NOLOGGING;

ALTER TABLE k_job_custom_to_restart
   ADD CONSTRAINT xpk_job_custom_to_restart PRIMARY KEY (job) USING INDEX;
...
CREATE OR REPLACE PACKAGE BODY apex_utils IS 
...
function f_dba_check return varchar2 is
--************************************************************
-- Funzione che serve per validare i package dopo un upgrade
--************************************************************
begin
	 return 'OK';
end f_dba_check;
